...
test('Open school setup branding on settings', async ({ page }) => {
  ...
  await expect(
    page.getByText(/Shown on login, sidebar, and reports|Upload school logo|PNG, JPEG, or WebP/i).first()
  ).toBeVisible();
  await expect(page.locator('main').getByText('Administrator')).toBeVisible();
});
